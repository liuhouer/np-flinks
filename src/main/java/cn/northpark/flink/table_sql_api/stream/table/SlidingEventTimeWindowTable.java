...
/**
 * 利用table api 结合事件时间划分滑动窗口
 */
public class SlidingEventTimeWindowTable {
    public static void main(String[] args) throws Exception {
        ...
        tableEnv.registerDataStream("t_orders", watermarksRow,
                "atime,uid,pid,money,rowtime.rowtime");

        Table table = tableEnv.scan("t_orders")
                .window(Slide.over("10.seconds").every("2.seconds").on("rowtime").as("window"))
                .groupBy("uid,window")
                .select("uid,window.start,window.end,window.rowtime,money.sum as total");

        tableEnv.toAppendStream(table, Row.class).print();
        env.execute("SlidingEventTimeWindowTable");
    }
}
